...
async def main() -> None:
    ...
    try:
        ...
        await mem.initialize_run_memory(state)
        ...
        print("\n==> Final MEMORY per agent (last 3 day entries each):")
        for agent in state.agents:
            content = await mem.read_memory(state, agent.persona.id)
            if not content:
                content = "<missing>"
            ...
            print(f"\n--- {agent.persona.display_name} ({agent.persona.id}) ---")
        ...
        fm = first_day_temperature(state.model_dump())
